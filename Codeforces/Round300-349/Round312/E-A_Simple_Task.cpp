...
int main() {
    int n, q; cin >> n >> q;
    string s; cin >> s;
    vector<lazy_segment_tree<RURS>> segs(26, lazy_segment_tree<RURS>(n, RURS::type1(0, 1)));
    for(int i = 0; i < n; ++i) {
        segs[s[i] - 'a'].update(i, i + 1, 1);
    }
    ...
    while(q--) {
        ...
        if(k == 1) {
            int cur = l;
            for(int i = 0; i < 26; ++i) {
                const int sum = segs[i].query(l, r).val;
                segs[i].update(l, r, 0);
                segs[i].update(cur, cur + sum, 1);
                cur += sum;
            }
        } else {
            int cur = l;
